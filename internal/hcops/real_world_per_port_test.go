...
func TestRealWorldPerPortConfiguration(t *testing.T) {
	...
	// - HTTP on port 80
	// - HTTPS on port 443 with specific certificates
	// - TCP on port 9000
	...
	service := &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				string(annotation.LBSvcProtocol):               "tcp",                              // Global default
				string(annotation.LBSvcProtocolPorts):         "80:http,443:https,9000:tcp",       // Per-port protocols
				string(annotation.LBSvcHTTPCertificatesPorts): "443:web-cert,api-cert",           // Per-port certificates
			},
		},
